...
const valVotingPower int64 = 1_000_000_000_000_000_000
...
it enables developers to configure their local environments to reflect mainnet conditions more accurately.`
	...
	cmd.Example = `kudorad in-place-testnet kudora_12000-1 <kudovaloper-address> --home $HOME/.kudora/validator1 --validator-privkey=<base64-validator-privkey> --accounts-to-fund="<kudo-account-1>,<kudo-account-2>"`
...
func newTestnetApp(logger log.Logger, db dbm.DB, appOpts servertypes.AppOptions) servertypes.Application {
	...
	newApp := newApp(logger, db, io.Discard, appOpts)
...
func initAppForTestnet(app *app.App, args valArgs) *app.App {
	...
	defaultCoins := sdk.NewCoins(sdk.NewInt64Coin(bondDenom, 1_000_000_000_000_000_000))
