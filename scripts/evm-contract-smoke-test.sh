...
if [[ "${USE_EXISTING_NODE}" == "1" ]]; then
  ...
  WORK_DIR="${WORK_ROOT}/evm-contract-smoke"
  HOME_DIR="${KUDORA_HOME:-}"
  COMET_RPC_URL="${KUDORA_RPC_URL:-http://127.0.0.1:26657}"
  JSONRPC_URL="${KUDORA_EVM_RPC_URL:-http://127.0.0.1:8545}"
else
  HOME_DIR="${ROOT_DIR}/tmp/phase-4-evm-contract-smoke"
  WORK_DIR="${HOME_DIR}"
  COMET_RPC_PORT="${KUDORA_EVM_CONTRACT_SMOKE_COMET_RPC_PORT:-26677}"
  JSONRPC_PORT="${KUDORA_EVM_CONTRACT_SMOKE_JSONRPC_PORT:-8549}"
  JSONRPC_WS_PORT="${KUDORA_EVM_CONTRACT_SMOKE_JSONRPC_WS_PORT:-8550}"
...
command -v jq >/dev/null 2>&1 || {
  echo "evm-contract-smoke-test: jq is required" >&2
  exit 1
}

command -v curl >/dev/null 2>&1 || {
  echo "evm-contract-smoke-test: curl is required" >&2
  exit 1
}

if [[ "${USE_EXISTING_NODE}" != "1" && ! -x "${BINARY}" ]]; then
  echo "evm-contract-smoke-test: expected built binary at ${BINARY}. Run make build first." >&2
  exit 1
fi
...
if [[ "${USE_EXISTING_NODE}" == "1" ]]; then
  ...
  [[ -f "${SENDER_KEY_FILE}" ]] || {
    echo "evm-contract-smoke-test: existing-node mode requires KUDORA_EVM_SENDER_KEY_FILE" >&2
    exit 1
  }
  [[ -f "${SENDER_INFO_FILE}" ]] || {
    echo "evm-contract-smoke-test: existing-node mode requires KUDORA_EVM_SENDER_INFO_FILE" >&2
    exit 1
  }
else
...
if [[ "${USE_EXISTING_NODE}" != "1" ]]; then
  ...
  if [[ -z "${validator_address}" ]]; then
    echo "evm-contract-smoke-test: could not derive validator address" >&2
    exit 1
  fi
...
if ! curl -sf "${COMET_RPC_URL}/health" >/dev/null; then
  echo "evm-contract-smoke-test: CometBFT RPC never became healthy" >&2
...
if ! printf '%s\n' "${chain_id_response}" | jq -e --arg expected "${EXPECTED_ETH_CHAIN_ID}" '.error == null and .result == $expected' >/dev/null; then
  echo "evm-contract-smoke-test: eth_chainId did not return ${EXPECTED_ETH_CHAIN_ID}" >&2
...
if ! printf '%s\n' "${block_number_response}" | jq -e '.error == null and (.result // "" | test("^0x[0-9a-f]+$")) and .result != "0x0"' >/dev/null; then
  echo "evm-contract-smoke-test: eth_blockNumber never reached a non-zero height" >&2
  printf 'last response: %s\n' "${block_number_response:-<empty>}" >&2
  [[ -f "${LOG_DIR}/start.stderr" ]] && tail -n 80 "${LOG_DIR}/start.stderr" >&2 || true
  exit 1
fi

"${HELPER_BIN}" contract-smoke \
  --rpc-url "${JSONRPC_URL}" \
  --chain-id "${EVM_CHAIN_ID}" \
  --sender-key-file "${SENDER_KEY_FILE}" \
  --result-file "${RESULT_FILE}" \
  >"${LOG_DIR}/contract.stdout" 2>"${LOG_DIR}/contract.stderr"

jq -e '
  .deployment_receipt_status == "0x1" and
  .store_receipt_status == "0x1" and
  .gas_used_deploy > 0 and
  .gas_used_store > 0 and
  .nonce_after_deploy == (.nonce_before + 1) and
  .nonce_after_store == (.nonce_after_deploy + 1) and
  .updated_value == "888"
' "${RESULT_FILE}" >/dev/null

echo "evm-contract-smoke-test: PASS (sender=${sender_eth_address} contract=$(jq -r '.contract_address' "${RESULT_FILE}") deployTx=$(jq -r '.deployment_tx_hash' "${RESULT_FILE}"))"
