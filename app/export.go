...
func (app *App) prepForZeroHeightGenesis(ctx sdk.Context, jailAllowedAddrs []string) error {
	...
	if err := iter.Close(); err != nil {
		app.Logger().Error("error while closing the reverse iterator", "error", err)
		return nil
	}
	...
	if err := app.SlashingKeeper.IterateValidatorSigningInfos(ctx, func(addr sdk.ConsAddress, info slashingtypes.ValidatorSigningInfo) (stop bool) {
		info.StartHeight = 0
